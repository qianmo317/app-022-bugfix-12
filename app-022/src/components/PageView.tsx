...
/** 行内各字块的 unit 区间，用于点击命中（块宽 = 小格数 × 100，累加定位） */
function blockRanges(row: Row): { char: string; start: number; end: number }[] {
  let x = 0;
  return row.map((b) => {
    const start = x;
    x += b.cells.length * 100;
    return { char: b.char, start, end: x };
  });
}
...
export const PageView = memo(function PageView({
  ...
  const sheetStyle = {
    width: `${PAGE.wMm}mm`,
    height: `${PAGE.hMm}mm`,
    padding: `${PAGE.marginTMm}mm ${PAGE.marginRMm}mm ${PAGE.marginBMm}mm ${PAGE.marginLMm}mm`,
  };
    ...
    <div className={className} data-pages data-page-count={pages.length}>
      {pages.map((rows, pi) => (
        <div key={pi} className="sheet" data-page={pi} style={sheetStyle}>
          ...
          <div className="sheet-rows" style={{ display: 'flex', flexDirection: 'column', gap: `${layout.lineGapMm}mm` }}>
            {rows.map((row, ri) => (
              <svg
                key={ri}
                className="row-svg"
